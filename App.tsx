...
function App() {
  ...
  const handleSendMessage = useCallback(async (text: string, attachment?: Attachment) => {
    ...
    if (isPdfRequest) {
        ...
        try {
            ...
            setMessages(prev => prev.map(msg => 
             ...
             setMessages(prev => prev.map(msg => msg.id === botMsgId ? { ...msg, text: "Erro ao gerar PDF.", isStreaming: false } : msg));
        } finally {
            setIsLoading(false);
            setIsPdfMode(false);
        }
    ...
    const isImageRequest = text.toLowerCase().includes('crie uma imagem') || 
                           ...
                           (attachment && (text.toLowerCase().includes('editar') || text.toLowerCase().includes('adicione') || text.toLowerCase().includes('mude') || text.toLowerCase().includes('transforme')));
    ...
    if (isImageRequest) {
        ...
        try {
           ...
           // Attempt generation/editing (Fail Fast if API error)
           // CRITICAL: Pass the attachment to generateImage for editing logic
           ...
           setLibrary(l => [...l, newItem]);
            ...
            setLoadingProgress(0);
            console.error(e);
            setMessages(prev => prev.map(m => m.id === botMsgId ? {...m, text: "Não foi possível gerar a imagem. Verifique se o prompt não viola as diretrizes ou tente novamente.", isStreaming: false} : m));
        } finally {
            setIsLoading(false);
        }
        return;
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: uuidv4(), role: Role.Model, text: "Desculpe, ocorreu um erro. Tente novamente.", isStreaming: false }
      ]);
    } finally {
       setMessages((prev) => 
          prev.map((msg) => 
